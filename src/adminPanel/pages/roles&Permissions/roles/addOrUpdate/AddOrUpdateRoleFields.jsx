    ...
    try {
      if (id) {
        ...
        navigate("/admin/role/view", {
          state: { message: "Role successfully updated!", severity: "success" },
        });
      } else {
        ...
        navigate("/admin/role/view", {
          state: { message: "Role successfully added!", severity: "success" },
        });
      }
    } catch (error) {
      setSnackbar({
      ...
      <form fullWidth onSubmit={handleSubmit(onSubmit)}>
        <Stack spacing={{ xs: 1, sm: 2, md: 4 }}>
          <Stack width={"100%"}>
            ...
            />
            {errors.name && (
              <Typography fontSize={"14px"} color={"#ff6384"} mt={".5rem"}>
                *{errors.name.message}
              </Typography>
            )}
          </Stack>
