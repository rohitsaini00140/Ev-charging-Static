...
} from "../../../../globalState/user/userApis";
  ...
  useEffect(() => {
    if (id && userForUpdate) {
      reset({
        ...
        cluters_id: userForUpdate.cluters_id || 0,
      });
    } else {
      reset(defaultValues);
    }
          ...
          >
            <Stack width={"100%"}>
              <SearchableDropdown
                ...
                value={watch("cluters_id") || 0}
                onChange={(newValue) =>
                  setValue("cluters_id", newValue, { shouldValidate: true })
                }
              />
              {errors.cluters_id && (
                <Typography color={"#ff6384"} fontSize={"13px"} mt={".5rem"}>
                  *{errors.cluters_id.message}
                </Typography>
              )}
            </Stack>
            ...
            <Stack width={"100%"}>
              <SearchableDropdown
                ...
                onChange={(newValue) =>
                  setValue("role_id", newValue, { shouldValidate: true })
                }
              />
              {errors.role_id && (
                <Typography color={"#ff6384"} fontSize={"13px"} mt={".5rem"}>
